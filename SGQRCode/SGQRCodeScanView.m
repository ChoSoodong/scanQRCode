...
- (void)drawRect:(CGRect)rect {
    ...
    // 设置空白区
    UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRect:CGRectMake(borderX + 0.5 * self.borderLineWidth, borderY + 0.5 *self.borderLineWidth, borderW - self.borderLineWidth, borderH - self.borderLineWidth)];
    [bezierPath fill];
    // 执行混合模式
    ...
    CGFloat insideExcess = fabs(0.5 * (self.cornerWidth - self.borderLineWidth));
    CGFloat outsideExcess = 0.5 * (self.borderLineWidth + self.cornerWidth);
    ...
    } else if(self.cornerLocation == CornerLoactionDefault){
